...
// Write collects the first remaining bytes of p and drops the rest without
// blocking the upstream writer; it returns len(p) to simulate a full write
// so callers like io.Copy never fail on truncation.
...
// Truncated reports whether truncation has occurred, for diagnosing
