...
// Config holds the remediation execution configuration.
// Config 处置执行配置。
type Config struct {
...
// MaxTries is the remediation attempt limit: confirmed findings still
// unremediated at the limit are escalated to a manual work order.
// MaxTries 处置尝试上限：达限仍未处置的 confirmed 升级人工工单（不静默、
// 不降级）。autopilot 引用本常量（原 maxRespondTries）。
const MaxTries = 3

// Plan is the remediation plan generated by the model for a single
// confirmed finding.
...
// PlanAction is a single remediation action.
// PlanAction 单个处置动作。
...
// Validate checks the three-piece contract and returns the names of
// actions that fail it.
...
// Suggestions extracts remediation suggestions (command plus rationale
// summary) for the report's "suggested fixes" section.
